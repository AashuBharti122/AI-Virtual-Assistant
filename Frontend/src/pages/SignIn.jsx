...
const handleSignIn=async (e)=>{
  ...
  return (
    ...
    style={{ backgroundImage: `url(${bgImage})` }}>
        ...
        <form className='w-[90%] h-[600px] max-w-[500px] bg-[#00000029] backdrop-blur 
        shadow-lg shadow-black flex flex-col justify-center items-center gap-[20px] 
        rounded-lg px-[20px]' onSubmit={handleSignIn}>

            <h1 className='text-[30px] font-semibold text-white mb-[30px]'>Sign In to 
                <span className='text-blue-400'>Virtual Assistant</span></h1>

            <input type="email" placeholder='Email' className='w-full h-[60px] bg-transparent 
            border-2 border-white outline-none text-white placeholder-gray-300 px-[20px] 
            py-[10px] rounded-full text-[18px]' required onChange={(e)=>setEmail(e.target.value)} 
            value={email}/>

            <div className='w-full h-[60px] bg-transparent border-2 border-white
             text-white rounded-full text-[18px] relative'>

                <input type={showPassword ? "text" : "password"} placeholder='Password' 
                className='w-full h-full rounded-full outline-none bg-transparent
                 placeholder-gray-300 px-[20px] py-[10px]'required onChange={(e)=>
                 setPassword(e.target.value)} value={password} />

                {!showPassword && <IoEye className='absolute top-[17px] right-[20px]
                 text-white cursor-pointer w-[25px] h-[25px]' onClick={() => 
            ...
            rounded-full mt-[30px] text-[20px] cursor-pointer' disabled={loading}>{loading ? 
            "Signing In..." : "Sign In"}</button>

            <p className='text-white text-[18px]'>Want to create a new Account ? 
                <span className='text-blue-400 cursor-pointer'   onClick={() => 
                    navigate("/signup")}>Sign Up</span></p>
